...
def rotate(img_matrix):
    temp = copy.deepcopy(img_matrix)
    for i in range(len(img_matrix)):
        for c in range(len(img_matrix[i])):
            img_matrix[i][c] = temp[len(img_matrix) - i - 1][len(img_matrix[i]) - c - 1]
    return img_matrix
...
def edge_detect(img_matrix):
    temp = copy.deepcopy(img_matrix)
    if len(img_matrix) >= 3 and len(img_matrix[0]) >= 3:
        for i in range(len(img_matrix)):
            for c in range(len(img_matrix[i])):
                if i != 0 and i != len(img_matrix) - 1 and c != 0 and c != len(img_matrix[i]) - 1:
                    for z in range(3):
                        x = temp[i][c][z] * 8
                        if(x - temp[i - 1][c + 1][z] - temp[i - 1][c][z] - temp[i - 1][c - 1][z] - temp[i][c + 1][z] -
                                           temp[i][c - 1][z] - temp[i + 1][c + 1][z] - temp[i + 1][c][z] - temp[i + 1][c - 1][z]) > 255:
                            img_matrix[i][c][z] = 255
                        elif (x - temp[i - 1][c + 1][z] - temp[i - 1][c][z] - temp[i - 1][c - 1][z] - temp[i][c + 1][z] -
                                           temp[i][c - 1][z] - temp[i + 1][c + 1][z] - temp[i + 1][c][z] - temp[i + 1][c - 1][z]) < 0:
                            img_matrix[i][c][z] = 0
                        else:  
                            img_matrix[i][c][z] = (x - temp[i - 1][c + 1][z] - temp[i - 1][c][z] - temp[i - 1][c - 1][z] - temp[i][c + 1][z] -
                                           temp[i][c - 1][z] - temp[i + 1][c + 1][z] - temp[i + 1][c][z] - temp[i + 1][c - 1][z])
    else:
         for i in range(len(img_matrix)):
            for c in range(len(img_matrix[i])):
                for z in range(3):
