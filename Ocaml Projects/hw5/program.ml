...
type expr =
  ...
  | Readint
  | ListC of int list
  | Cons of expr * expr (*added cases based off instructions*)
  | Head of expr
  | Tail of expr
  | Add of expr * expr
  | Sub of expr * expr
  ...
  | Apply of expr * expr
  | Print of expr
 and expType = IntT | BoolT | ListT | UnitT | FunT of expType * expType (*added listT to expType*)
...
let rec eval exp state = match exp with
  ...
  | IntC n -> (IntR n, state)
  | ListC n -> (ListR n, state)
  | Head n -> (match eval n state with (ListR (h::t), st) -> (IntR h, st))
  | Tail n -> (match eval n state with (ListR (h::t), st) -> (ListR t, st))
  | BoolC b -> (BoolR b, state)
  | Cons (e1,e2) -> let (IntR i1, st1) = eval e1 state in
                    let (ListR i2, st2) = eval e2 st1 in
                    ListR (i1::i2), st2
  ...
  | Print e -> let (r,st') = eval e state in
     ...
     | ListR x -> print_string ("["^(String.concat " " (List.map string_of_int x))^"]")
...
let rec typeof exp env = match exp with
  ...
  | Head e -> (match (typeof e env) with
    | ListT -> IntT
    | _ -> type_err "Head")
  | Tail e -> (match (typeof e env) with (* type judgement: e : ListT => Tail e : ListT *)
    | ListT -> ListT
    | _ -> type_err "Tail")
  | Cons (e1,e2) -> ( match (typeof e1 env, typeof e2 env) with (* type judgement: e1 : IntT, e2: ListT => Cons e1,e2 : ListT *)
    | (IntT,ListT) -> ListT
    | (ListT, ListT) -> ListT
    | _ -> type_err "Cons")
  | IntC _ -> IntT
  ...
  | Eq (e1,e2) ->
     ( match (typeof e1 env, typeof e2 env) with
       | (IntT,IntT) | (ListT,ListT) | (BoolT,BoolT) | (UnitT,UnitT) -> BoolT
