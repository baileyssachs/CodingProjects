...
(*added a ton of tokens based off what we need*)
...
let tokenize_string = function (*added un string forms of tokens i added above*)
  | "(" -> OP
  | ")" -> CP
  | "and" -> AND
  | "or" -> OR
...
| (ID s)::t -> (Name s, t)
| READ::t -> (Readint, t)
| LB::t -> let (l,t') = _parse_intlist t in (ListC(l),t') (* this one goes to my own function that I made to handle brackers*)
...
(*function used to go through brackets*)
and _parse_intlist t =  match t with
| RB::t' -> ([],t') (* if right bracker then return the test of the list*)
| [] -> synerr "unclosed list" (*if no right bracket it never closed*)
| (ICONST x)::t' -> let (e1,t1) = _parse_intlist t' in (* make sure all things in list are consts*)
                        (x::e1, t1)
| _ -> synerr "list literal" (* anything else is a list literal*)
